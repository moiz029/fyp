...
try:
    ...
    mongo = flask_pymongo.MongoClient(
        "mongodb://my_team:X3Njg1cRrj9QT5Ks@ac-ga3haug-shard-00-00.59spobs.mongodb.net:27017,ac-ga3haug-shard-00-01.59spobs.mongodb.net:27017,ac-ga3haug-shard-00-02.59spobs.mongodb.net:27017/?ssl=true&replicaSet=atlas-ugvkvk-shard-0&authSource=admin&retryWrites=true&w=majority")
    db = mongo.my_team
    mongo.server_info()
except:
...
def login_franchise(login_credentionals):
    franchise = db.franchises.find(
        {"franchise_id": login_credentionals['franchise_id'], 'password': login_credentionals['password']})
    ...
    try:
        ...
        db.sessions.insert_one(
            {'session_id': session, 'user_previliges': 'franchise', "user_id": franchise["franchise_id"]})
...
def select_player(data, session):
    ...
    db.franchises.find_one_and_update({"franchise_id": franchise["franchise_id"]},
                                      {'$set': {"draft": franchise["draft"], "team": franchise["team"]}})
...
def select_playing_xi_player(data, session):
    ...
    db.franchises.find_one_and_update({"franchise_id": franchise["franchise_id"]},
                                      {'$set': {"playing_xi": franchise["playing_xi"]}})
...
def change_availability(data, session):
    ...
    franchise["draft"].remove(
        next(fplayer for fplayer in franchise['draft'] if fplayer["playerid"] == player['playerid']))
    franchise["draft"].append(player)
    db.franchises.find_one_and_update({"franchise_id": franchise["franchise_id"]},
                                      {'$set': {"draft": franchise["draft"]}})
